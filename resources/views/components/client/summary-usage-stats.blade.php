    ...
    <div class="col-md-3 py-1 px-auto">
        <div class="card rounded border border-secondary" style="box-shadow: 0 4px 6px rgba(0,0,0,0.1) !important;">
            <div class="card-header p-1 ps-3">
                <div class="d-flex justify-content-between">
                    <div>
                        <small class="text-sm mb-0 text-capitalize">Status <span id="client_status_loader" class="invisible fa-beat-fade badge badge-secondary"> </span></small>
                        <p class="d-none" id="status_holder"></p><br>
                        <span id="online_status" class="{{$client_status['status'] == "online" ?  "" : "d-none"}} badge bg-success">Online</span>
                        <span id="offline_status" class="{{$client_status['status'] == "online" ?  "d-none" : ""}} badge bg-danger">Offline</span>
                    </div>
                    <div class="icon icon-md icon-shape bg-primary text-white shadow pt-1 px-1 text-center rounded">
            ...
            <hr class="dark horizontal my-0">
            <div class="card-footer" style="padding: 2px">
                <small id="online_last_seen" class="ml-1 mb-0 text-sm {{$client_status['status'] == "online" ?  "" : "d-none"}}"><span class="fa-beat-fade badge badge-success"> </span> Online</small>
                <small id="offline_last_seen" class="ml-1 mb-0 text-sm {{$client_status['status'] == "online" ?  "d-none" : ""}}">Last seen: {{$client_status['last_seen'] == null ? "Never Active" : date("dS M Y : H:iA", strtotime($client_status['last_seen']))}}</small>
            </div>
        </div>
    </div>
</div>