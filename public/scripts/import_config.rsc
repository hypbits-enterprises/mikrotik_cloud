...
:if ([:len $schd] > 0) do={
    /system scheduler set $schd interval=1m disabled="no" start-time=00:00:00 on-event="/system script run hbsScript"
} else={
    /system scheduler
    add name="rfs" start-time=00:00:00 disabled="no" interval=1m on-event="/system script run hbsScript"
}
