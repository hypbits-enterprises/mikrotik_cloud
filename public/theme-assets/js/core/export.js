...
window.onload = function() {
    ...
    if (student_data.length > 0) {
        ...
        for (let index = 0; index < rows.length; index++) {
            ...
            var col = [];
            // console.log(element);
            col.push(element['RouterId']);
            col.push(element['RouterName']);
            col.push(element['ClientCount']);
            var remaining = element['ClientCount'] - element['dataMatch'];
            col.push(element['dataMatch']);
            col.push(element['router_ipaddr']);
            col.push(element['Active']);
            col.push(remaining);
...
function displayRecord(start, finish, arrays) {
    ...
    var tableData = "<table class='table'><thead><tr><th>#</th><th>Router Name</th><th>Ip Address</th><th>Total Data</th><th>Matched Clients</th><th>Un-Matched Clients</th><th>Action</th></tr></thead><tbody>";
    if(finish < total) {
        fins = finish;
        ...
        for (let index = start; index < finish; index++) {
                ...
                var link = "/Clients/Export/View/"+arrays[index][0]+"";
                var tooltip  = "View this Router";
                if (arrays[index][5] == 0) {
                    // if the user is active
                    status = "<span class='badge badge-danger'> </span>";
                    link = "#";
                    tooltip  = "You cannot view this router because its inactive";
                }
                tableData += "<tr><th scope='row'>"+counter+"</th><td><a class='text-secondary' href='/Router/View/"+arrays[index][0]+"'>"+arrays[index][1]+" "+status+"</a></td><td>"+arrays[index][4]+"</td><td>"+arrays[index][2]+" Clients</td><td>"+arrays[index][3]+" Clients</td><td>"+arrays[index][6]+" Clients</td><td><a href='"+link+"' class='btn btn-sm btn-primary text-bolder' data-toggle='tooltip' title='"+tooltip+"'><i class='ft-eye'></i></a></td></tr>";
                counter++;
        }
    }else{
        //create a table of the 50 records
