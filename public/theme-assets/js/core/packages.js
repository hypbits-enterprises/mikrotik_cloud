...
window.onload = function() {
    ...
    if (student_data.length > 0) {
        ...
        for (let index = 0; index < rows.length; index++) {
            ...
            var col = [];
            // console.log(element);
            col.push(element['package_id']);
            col.push(element['package_name']);
            col.push(element['free_trial_period']);
            col.push(element['payment_intervals']);
            col.push(element['amount_to_pay']);
        ...
        cObj("transDataReciever").innerHTML = "<p class='sm-text text-danger text-bold text-center'><span style='font-size:40px;'><i class='ft-alert-triangle'></i></span> <br>Ooops! No packages found!</p>";
...
function displayRecord(start, finish, arrays) {
    ...
    var tableData = "<table class='table'><thead><tr><th>#</th><th>Package Name</th><th>Free Trial Period</th><th>Payments Interval</th><th>Package Price</th><th>Action</th></tr></thead><tbody>";
    if(finish < total) {
        fins = finish;
