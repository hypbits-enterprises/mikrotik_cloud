...
function displayRecord(start, finish, arrays) {
    ...
    if(finish < total) {
        ...
        for (let index = start; index < finish; index++) {
            ...
            var actions = "<a href='/Transactions/View/" + arrays[index][6] + "' class='btn btn-sm btn-primary text-bolder "+readonly+"' data-toggle='tooltip' title='View this transaction' style=\"padding: 3px;\" id=\"\" data-toggle=\"tooltip\" title=\"\" ><span class=\"d-inline-block border border-white w-100 text-center\" style=\"border-radius: 2px; padding: 5px;\"><i class='ft-eye'></i> View</span></a><a class='btn btn-sm btn-info ml-1 "+readonly+"' data-toggle='tooltip' title='Print receipt' target='_blank' href='/Print-Reciept/"+arrays[index][6]+"' style=\"padding: 3px;\"><span class=\"d-inline-block border border-white w-100 text-center\" style=\"border-radius: 2px; padding: 5px;\"><i class=\"ft-printer\"></i></span></a>";
...
cObj("transaction_reports_btn").onclick = function () {
    cObj("show_generate_reports_window").classList.toggle("hide");
}

cObj("transaction_date_option").onchange = function () {
