...
window.onload = function () {
    ...
    if (student_data.length > 0) {
        ...
        for (let index = 0; index < rows.length; index++) {
            ...
            col.push(element['problem']);
            col.push(element['solution']);
            col.push(element['diagnosis']);
            col.push(element['closed_by']);
...
function sort_by_network() {
    ...
    if (sort_by_net == 0) {
        ...
        cObj("sort_by_network_gateway").innerHTML = "Network & Gateway <i class='ft-chevron-up'></i>";
    }
    if (rowsColStudents.length > 0) {
        cObj("sort_by_reg_date").addEventListener("click", sortByRegDate);
        cObj("sort_by_name").addEventListener("click", sortByName);
...
function displayRecord(start, finish, arrays) {
    ...
    if (finish < total) {
        ...
        for (let index = start; index < finish; index++) {
            ...
            tableData += "<tr><th scope='row'><input type='checkbox' class='actions_id' id='actions_id_"+arrays[index][11]+"'><input type='hidden' id='actions_value_"+arrays[index][11]+"' value='"+arrays[index][11]+"'> " + counter + "</th><td>" + assignment + " <a href='/Clients/View/" + arrays[index][0] + "' class='text-secondary'>" + ucwords(arrays[index][1]) + " " + status + "</a><br><small class='text-gray d-none d-xl-block'>" + ucword(arrays[index][13]) + "</small></td><td data-html='true' data-toggle='tooltip' title='' data-original-title='"+(arrays[index][17] != null ? "<b>Last Report :</b> "+arrays[index][17]+" <br> <b>Problem : </b>"+arrays[index][24]+"<br> <b>Diagnosis : </b>"+arrays[index][26]+"<br> <b>Solution : </b>"+arrays[index][25]+"<br> <b>Comment : </b>"+arrays[index][18]+"<br> <b>Status : </b> "+(arrays[index][21] == "cleared" ? "RESOLVED" : arrays[index][21].toUpperCase())+"<br><b>Date Reported</b> : "+arrays[index][19]+"<br><b>Closed By : </b>"+(arrays[index][27] ? arrays[index][27] : "Not Set!")+"" : "No Issues Yet!")+"'>" + (arrays[index][17] != null ? "<a href='/Client-Reports/View/" + arrays[index][23] + "' class='text-secondary'>"+arrays[index][11].toUpperCase()+"</a>" : arrays[index][11].toUpperCase()) + " " + reffered + "</td><td>" + ucwords(arrays[index][5]) + "<br><small class='d-none d-md-block'>" + location + "</small></td><td>" + setDate(arrays[index][7]) + "</td><td><small>"+arrays[index][2]+" <br> "+arrays[index][16]+" </small> <br> <small class='text-gray d-xl-block'>{" + ucword(arrays[index][22]) + "}</small></td><td><a href='/Clients/View/" + arrays[index][0] + "' class='btn btn-sm btn-primary text-bolder' data-toggle='tooltip' title='View this User'><i class='ft-eye'></i></a> <a href='/deactivate/" + arrays[index][0] + "' class='btn btn-sm btn-danger text-dark text-bolder "+readonly_flag+"'  data-toggle='tooltip' title='Disable this User'><i class='ft-x'></i></a></td></tr>";
            counter++;
        }
    } else {
        //create a table of the 50 records
