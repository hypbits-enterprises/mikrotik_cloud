...
window.onload = function () {
    ...
    if (student_data.length > 0) {
        ...
        for (let index = 0; index < rows.length; index++) {
            ...
            col.push(element['opened_by']);
            col.push(element['admin_attender']);
            col.push(element['validated']);
...
function displayRecord(start, finish, arrays) {
    ...
    if (finish < total) {
        ...
        for (let index = start; index < finish; index++) {
            ...
            var verification = "";
            if (arrays[index][30] == "1") {
                verification = "<span class='' data-toggle='tooltip' title='Verified'>"+ucwords(arrays[index][1])+"</span>";
            }else{
